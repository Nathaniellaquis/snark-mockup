        ...
        <Card variant="snarkGlassBold" className="mb-6">
          <CardContent className="pt-8">
            <div className="relative mb-6">
              ...
              <Input placeholder="Search users..." className="h-14 pl-12" />
            </div>
              ...
              {users.map((user) => (
                <div key={user.id} className="p-5 bg-white/5 border border-white/10 rounded-xl flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-3">
                    <Badge variant={user.role === "admin" ? "snarkFuchsia" : user.role === "moderator" ? "snarkViolet" : "snarkGhost"}>
                      {user.role}
                    </Badge>
                    <Badge variant={user.status === "active" ? "snarkGreen" : "snarkGhost"}>
                      {user.status}
                    </Badge>
                  </div>
                </div>
